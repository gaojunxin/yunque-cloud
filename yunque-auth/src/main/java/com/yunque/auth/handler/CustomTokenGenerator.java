...
public class CustomTokenGenerator implements OAuth2TokenGenerator<OAuth2AccessToken> {
    ...
    public OAuth2AccessToken generate(OAuth2TokenContext context) {
        if (ObjectUtil.notEqual(OAuth2TokenType.ACCESS_TOKEN, context.getTokenType()) || ObjectUtil.notEqual(OAuth2TokenFormat.REFERENCE, context.getRegisteredClient().getTokenSettings().getAccessTokenFormat()))
            return null;
        ...
        if (this.accessTokenCustomizer != null) {
            // @formatter:off
            OAuth2TokenClaimsContext.Builder accessTokenContextBuilder = OAuth2TokenClaimsContext.with(claimsBuilder).registeredClient(context.getRegisteredClient()).principal(context.getPrincipal()).authorizationServerContext(context.getAuthorizationServerContext()).authorizedScopes(context.getAuthorizedScopes()).tokenType(context.getTokenType()).authorizationGrantType(context.getAuthorizationGrantType());
    ...
    /**
     * Sets the {@link org.springframework.security.oauth2.server.authorization.token.OAuth2TokenCustomizer} that customizes the
     * {@link OAuth2TokenClaimsContext#getClaims() claims} for the
     * {@link OAuth2AccessToken}.
     *
     * @param accessTokenCustomizer the {@link org.springframework.security.oauth2.server.authorization.token.OAuth2TokenCustomizer} that customizes the
